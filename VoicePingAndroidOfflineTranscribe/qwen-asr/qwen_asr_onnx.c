...
#define ONNX_LOGI(...) __android_log_print(ANDROID_LOG_INFO, "QwenOnnx", __VA_ARGS__)
#define ONNX_LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "QwenOnnx", __VA_ARGS__)
#else
#define ONNX_LOGI(...) do { if (qwen_onnx_verbose) fprintf(stderr, __VA_ARGS__); } while(0)
#define ONNX_LOGE(...) fprintf(stderr, __VA_ARGS__)
#endif

int qwen_onnx_verbose = 0;

/* ======================================================================== */
/* Android: dynamically load ORT via dlopen                                  */
/* ======================================================================== */
#ifdef __ANDROID__
typedef const OrtApiBase* (*OrtGetApiBase_fn)(void);
static const OrtApi* android_load_ort_api(void) {
    void *handle = dlopen("libonnxruntime.so", RTLD_NOW | RTLD_GLOBAL);
    if (!handle) {
        ONNX_LOGE("Cannot load libonnxruntime.so: %s\n", dlerror());
        return NULL;
    }
    OrtGetApiBase_fn get_base = (OrtGetApiBase_fn)dlsym(handle, "OrtGetApiBase");
    if (!get_base) {
        ONNX_LOGE("OrtGetApiBase not found: %s\n", dlerror());
        return NULL;
    }
    return get_base()->GetApi(ORT_API_VERSION);
}
#endif
...
static float *load_npy(const char *path, int *rows, int *cols) {
    ...
    if (!f) { ONNX_LOGE("qwen_onnx: cannot open %s\n", path); return NULL; }
...
#define ORT_CHECK(expr) do { \
    ...
    if (_s) { \
        ...
        ONNX_LOGE("qwen_onnx ORT error: %s\n", _m); \
...
qwen_onnx_ctx_t *qwen_onnx_load(const char *model_dir) {
...
#ifdef __ANDROID__
    const OrtApi *api = android_load_ort_api();
    if (!api) { free(ctx); return NULL; }
#else
    ...
    free(embed_path);
    if (!ctx->embed_tokens) {
        ONNX_LOGE("qwen_onnx: failed to load embed_tokens\n");
        qwen_onnx_free(ctx);
        return NULL;
    }
    ctx->vocab_size = rows;
    ctx->hidden_dim = cols;
    ONNX_LOGI("Embeddings: %d x %d\n", rows, cols);
    ...
    free(vocab_path);
    if (!ctx->tokenizer) {
        ONNX_LOGE("qwen_onnx: failed to load tokenizer\n");
        qwen_onnx_free(ctx);
        return NULL;
    }
...
char *qwen_onnx_transcribe(qwen_onnx_ctx_t *ctx, const float *samples, int n_samples) {
    ...
    ONNX_LOGI("Audio embeddings: %d tokens x %d dim\n", n_audio, (int)enc_shape[2]);
